...
app.post('/estoque/movimentar', verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Registra a movimentação com o idusuario
    await pool.query(
      'INSERT INTO MovimentacaoEstoque (idproduto, idusuario, tipo, quantidade, observacao) VALUES (?, ?, ?, ?, ?)',
      [idproduto, idusuario, tipo, quantidade, observacao || null]
    );

    res.send('Movimentação registrada com sucesso!');
  } catch (err) {
    console.error('Erro na movimentação:', err);
    res.status(500).send('Erro no servidor ao registrar movimentação.');
  }
});


app.get('/estoque/historico', verifyToken, async (req, res) => {
  const idusuario = req.userId;
  try {
    console.log('Buscando histórico para usuário:', idusuario);
...
app.delete('/estoque/historico/:idmovimentacao', verifyToken, async (req, res) => {
  try {
    ...
    await pool.query(
      'DELETE FROM MovimentacaoEstoque WHERE idmovimentacao = ?',
      [idmovimentacao]
    );

    res.status(200).send('Movimentação excluída com sucesso.');
  } catch (err) {
